/* neutrophils*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_neutrophils_rmoutlier.csv";
...
set neutrophils ;
LINEAR = TIME;
logneutrophils = log(neutrophils + 0.001);
run;
...
PROC SGPLOT NOAUTOLEGEND DATA=neutrophils;
	* observed trends;
	SERIES X=TIME Y=logneutrophils / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logneutrophils / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logneutrophils / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;

/* PART 1 */
PROC MEANS DATA=neutrophils NWAY MEAN STD;
	CLASS TIME;
	VAR logneutrophils;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logneutrophils=TIME  GROUP GROUP*TIME  /SOLUTION;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
	MODEL logneutrophils=TIME  GROUP GROUP*TIME  /SOLUTION outp=pred;
	REPEATED LINEAR /SUB=ID TYPE=UN R RCORR;
...
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/neutrophils.xlsx" REPLACE; 
RUN;