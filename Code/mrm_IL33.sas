/* IL33*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_IL33_2.csv";
...
PROC SGPLOT NOAUTOLEGEND DATA=IL33;
	* observed trends;
	SERIES X=TIME Y=logIL33 / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logIL33 / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logIL33 / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;

/* PART 1 */
PROC MEANS DATA=IL33 NWAY MEAN STD;
	CLASS TIME;
	VAR logIL33;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logIL33=TIME GROUP GROUP*TIME /SOLUTION;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logIL33=TIME GROUP GROUP*TIME /SOLUTION outp=pred;
	REPEATED LINEAR /SUB=ID TYPE=ARH(1) R RCORR;/*  */

PROC EXPORT
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/IL33.xlsx" REPLACE; 
RUN;
