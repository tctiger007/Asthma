/* IL13*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_IL13_2.csv";
...
PROC SGPLOT NOAUTOLEGEND DATA=IL13;
	* observed trends;
	SERIES X=TIME Y=logIL13 / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logIL13 / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logIL13 / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;

/* PART 1 */
PROC MEANS DATA=IL13 NWAY MEAN STD;
	CLASS TIME;
	VAR logIL13;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logIL13=TIME  GROUP GROUP*TIME/SOLUTION;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logIL13=TIME GROUP GROUP*TIME/SOLUTION outp=pred;
	REPEATED LINEAR /SUB=ID TYPE=UN R RCORR;
...
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/IL13.xlsx" REPLACE; 
RUN;
