/* eosinophils*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_eosinophils_rmoutlier.csv";
...
set eosinophils ;
LINEAR = TIME;
logeosinophils = log(eosinophils + 0.001);
run;
...
PROC SGPLOT NOAUTOLEGEND DATA=eosinophils;
	* observed trends;
	SERIES X=TIME Y=logeosinophils / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logeosinophils / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logeosinophils / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;

/* PART 1 */
PROC MEANS DATA=eosinophils NWAY MEAN STD;
	CLASS TIME;
	VAR logeosinophils;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logeosinophils=TIME  GROUP GROUP*TIME  /SOLUTION;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
	MODEL logeosinophils=TIME  GROUP GROUP*TIME  /SOLUTION outp=pred;
	REPEATED LINEAR /SUB=ID TYPE=CS R RCORR;
/*  */
PROC EXPORT
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/eosinophils.xlsx" REPLACE; 
RUN;