/* FeNO*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_FeNO.csv";
...
PROC SGPLOT NOAUTOLEGEND DATA=FeNO;
	* observed trends;
	SERIES X=TIME Y=logFeNO / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logFeNO / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logFeNO / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;

/* PART 1 */
PROC MEANS DATA=FeNO NWAY MEAN STD;
	CLASS TIME;
	VAR logFeNO;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logFeNO=TIME  GROUP GROUP*TIME  /SOLUTION;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logFeNO=TIME  GROUP GROUP*TIME  /SOLUTION outp=pred;
	REPEATED LINEAR /SUB=ID TYPE=UN R RCORR;
/* 	 */
/*  */
PROC EXPORT
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/FeNO.xlsx" REPLACE; 
RUN;