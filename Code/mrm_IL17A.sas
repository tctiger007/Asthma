/* IL17A*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_IL17A_2.csv";
...
PROC SGPLOT NOAUTOLEGEND DATA=IL17A;
	* observed trends;
	SERIES X=TIME Y=logIL17A / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logIL17A / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logIL17A / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;

/* PART 1 */
PROC MEANS DATA=IL17A NWAY MEAN STD;
	CLASS TIME;
	VAR logIL17A;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logIL17A=TIME TIME*TIME GROUP GROUP*TIME GROUP*TIME*TIME/SOLUTION;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logIL17A=TIME TIME*TIME GROUP GROUP*TIME GROUP*TIME*TIME/SOLUTION outp=pred;
	REPEATED LINEAR /SUB=ID TYPE=UN R RCORR;/*  */
PROC EXPORT
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/IL17A.xlsx" REPLACE; 
RUN;
