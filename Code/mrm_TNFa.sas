/* TNFa*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_TNFa_2.csv";
...
PROC SGPLOT NOAUTOLEGEND DATA=TNFa;
	* observed trends;
	SERIES X=TIME Y=logTNFa / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logTNFa / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logTNFa / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;

/* PART 1 */
PROC MEANS DATA=TNFa NWAY MEAN STD;
	CLASS TIME;
	VAR logTNFa;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logTNFa=TIME GROUP GROUP*TIME /SOLUTION;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
MODEL logTNFa=TIME GROUP GROUP*TIME /SOLUTION outp=pred;
	REPEATED LINEAR /SUB=ID TYPE=UN R RCORR;/*  */

PROC EXPORT
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/TNFa.xlsx" REPLACE; 
RUN;
