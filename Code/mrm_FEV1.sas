/* FEV1PercentPred*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_FEV1PercentPred.csv";
...
data FEV1PercentPred;
set FEV1PercentPred ;
LINEAR = TIME;
logFEV1PercentPred = log(FEV1PercentPred);
run;
...
PROC SGPLOT NOAUTOLEGEND DATA=FEV1PercentPred;
	* observed trends;
	SERIES X=TIME Y=logFEV1PercentPred / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logFEV1PercentPred / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logFEV1PercentPred / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;


/* PART 1 */
PROC MEANS DATA=FEV1PercentPred NWAY MEAN STD;
	CLASS TIME;
	VAR logFEV1PercentPred;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
	MODEL logFEV1PercentPred=TIME GROUP GROUP*TIME/S;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
	MODEL logFEV1PercentPred=TIME GROUP GROUP*TIME/S outp=pred;
	RANDOM INTERCEPT TIME/SUB=ID TYPE=UN G GCORR;
	REPEATED LINEAR /SUB=ID R RCORR;
/*  */
Proc export
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/FEV1percentpred_individual.xlsx" REPLACE; 
RUN;
