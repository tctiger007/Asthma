/* FVCPercentPred*/
filename rf "/home/wwang750/Biofluc_testRVeffectiveness_cytokine/mrm_FVCPercentPred.csv";
...
set FVCPercentPred ;
LINEAR = TIME;
logFVCPercentPred = log(FVCPercentPred);
run;
...
PROC SGPLOT NOAUTOLEGEND DATA=FVCPercentPred;
	* observed trends;
	SERIES X=TIME Y=logFVCPercentPred / GROUP=ID LINEATTRS=(THICKNESS=1);
	*overall linear ;
	REG X=TIME Y=logFVCPercentPred / NOMARKERS LINEATTRS=(COLOR=BLUE PATTERN=1 THICKNESS=3);
	*overall spline;
	PBSPLINE X=TIME Y=logFVCPercentPred / NOMARKERS LINEATTRS=(COLOR=RED THICKNESS=3);
RUN;


/* PART 1 */
PROC MEANS DATA=FVCPercentPred NWAY MEAN STD;
	CLASS TIME;
	VAR logFVCPercentPred;
RUN;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
	MODEL logFVCPercentPred=TIME GROUP GROUP*TIME/S;
...
PROC MIXED METHOD=ML COVTEST;
	CLASS ID LINEAR;
	MODEL logFVCPercentPred=TIME GROUP GROUP*TIME/S outp=pred;
	RANDOM INTERCEPT TIME/SUB=ID TYPE=UN G GCORR;
	REPEATED LINEAR /SUB=ID R RCORR; 
...
DATA =work.pred
DBMS=xlsx
outfile="/home/wwang750/Biofluc_testRVeffectiveness_cytokine/Output/FVCpercentpred_individual.xlsx" REPLACE; 
RUN;
